...
import {
  Plus, Search, Edit2, Trash2, Save, X, Wifi, Car, Waves,
  Wind, Trees, Droplet, Sparkles, CheckCircle2, AlertCircle
...
const iconMap: Record<string, any> = Object.fromEntries(iconOptions.map(o => [o.value, o.icon]))
const getIcon = (v: string) => iconMap[v?.toLowerCase()] ?? Sparkles

const filtered = computed(() =>
  options.value.filter(o => o.name?.toLowerCase().includes(searchTerm.value.toLowerCase()))
)
...
const load = async () => {
  loading.value = true
  try { options.value = await getList('/options') } catch { options.value = [] }
...
const saveOption = async () => {
  ...
  saving.value = true
  try {
    const url    = editingId.value ? `${API_BASE}/options/${editingId.value}` : `${API_BASE}/options`
    const method = editingId.value ? 'PUT' : 'POST'
    const res    = await fetch(url, { method, headers: authHeaders(), body: JSON.stringify({ name: form.name.trim(), icon: form.icon, description: form.description }) })
...
const deleteOption = async (id: string) => {
  ...
  deleting.value = id
  try {
    const res = await fetch(`${API_BASE}/options/${id}`, { method: 'DELETE', headers: authHeaders() })
...
<template>
  <div class="space-y-6 p-6">
    ...
    <transition name="fade">
      <div v-if="toast" class="fixed right-6 top-6 z-50 flex items-center gap-3 rounded-2xl border px-5 py-3 shadow-xl font-semibold text-sm"
        :class="toast.type === 'ok' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-800'">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-muted-foreground">Créez et gérez les équipements des biens (Wi-Fi, Piscine...)</p>
      </div>
    ...
    <!-- Formulaire modal -->
    <transition name="fade">
      <div v-if="showForm" class="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="w-full max-w-md rounded-2xl bg-card border border-border shadow-2xl overflow-hidden">
          <div class="flex items-center justify-between border-b border-border p-5">
            <h2 class="font-bold text-foreground text-lg">{{ editingId ? 'Modifier' : 'Nouvelle' }} option</h2>
            <button @click="cancelForm" class="rounded-full p-1 hover:bg-muted transition-colors"><X class="h-5 w-5" /></button>
          </div>
          <div class="p-5 space-y-4">
            <div>
              <label class="mb-1.5 block text-sm font-semibold text-foreground">Nom de l'option *</label>
            ...
            <div>
              ...
              <div class="grid grid-cols-4 gap-2">
                <button v-for="opt in iconOptions" :key="opt.value" type="button"
                  ...
                  :class="form.icon === opt.value ? 'border-primary bg-primary/10 text-primary' : 'border-input hover:border-primary/50'">
            ...
            <div>
              <label class="mb-1.5 block text-sm font-semibold text-foreground">Description (optionnel)</label>
          ...
          <div class="flex gap-3 border-t border-border p-5">
            <button @click="cancelForm" class="flex-1 rounded-xl border border-input py-2.5 text-sm font-semibold hover:bg-muted transition-colors">Annuler</button>
    ...
    <div class="relative max-w-sm">
      ...
      <input v-model="searchTerm" placeholder="Rechercher une option..."
    ...
    <div v-else-if="filtered.length === 0" class="flex flex-col items-center py-16 text-center">
      ...
      <p class="font-semibold text-muted-foreground">Aucune option trouvée</p>
      <button @click="openCreate" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-bold text-primary-foreground hover:bg-primary/90 transition-colors">
        <Plus class="h-4 w-4" />Créer la première option
      </button>
    </div>

    <div v-else class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
      <div v-for="opt in filtered" :key="opt.id"
        class="group relative rounded-2xl border border-border bg-card p-5 shadow-sm hover:shadow-md hover:border-primary/30 transition-all">
        <div class="flex flex-col items-center gap-3 text-center">
          ...
          <div>
            ...
            <p v-if="opt.description" class="mt-0.5 text-xs text-muted-foreground line-clamp-1">{{ opt.description }}</p>
          </div>
        </div>
        <div class="mt-4 flex gap-2">
          ...
          <button @click="deleteOption(String(opt.id))" :disabled="deleting === String(opt.id)"
