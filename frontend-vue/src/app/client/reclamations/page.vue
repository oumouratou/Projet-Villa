...
const clientComplaints = computed(() => complaints.value.filter((c) => c.client?.email === currentUser.value?.email))

const filteredComplaints = computed(() => {
  return clientComplaints.value.filter((complaint) => {
...
const statusLabels: Record<string, string> = { nouvelle: 'Nouvelle', ouverte: 'Ouverte', en_cours: 'En cours', traitee: 'Traitee' }
  ...
  <main class="flex-1 overflow-auto p-6">
    <div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
      <div class="grid grid-cols-3 gap-4">
        <div class="rounded-xl border border-border bg-card p-4"><div class="flex items-center gap-3"><div class="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-100"><AlertCircle class="h-5 w-5 text-blue-600" /></div><div><p class="text-xl font-bold">{{ clientComplaints.filter(c => c.status === 'nouvelle').length }}</p><p class="text-xs text-muted-foreground">Nouvelles</p></div></div></div>
        <div class="rounded-xl border border-border bg-card p-4"><div class="flex items-center gap-3"><div class="flex h-10 w-10 items-center justify-center rounded-lg bg-yellow-100"><Clock class="h-5 w-5 text-yellow-600" /></div><div><p class="text-xl font-bold">{{ clientComplaints.filter(c => c.status === 'en_cours').length }}</p><p class="text-xs text-muted-foreground">En cours</p></div></div></div>
        <div class="rounded-xl border border-border bg-card p-4"><div class="flex items-center gap-3"><div class="flex h-10 w-10 items-center justify-center rounded-lg bg-green-100"><CheckCircle class="h-5 w-5 text-green-600" /></div><div><p class="text-xl font-bold">{{ clientComplaints.filter(c => c.status === 'traitee').length }}</p><p class="text-xs text-muted-foreground">Traitees</p></div></div></div>
      </div>
    ...
    <div class="mb-6 rounded-xl border border-border bg-card p-4">
      <div class="flex flex-col gap-4 sm:flex-row">
        ...
        <div class="flex items-center gap-2"><Filter class="h-5 w-5 text-muted-foreground" /><select v-model="statusFilter" class="rounded-lg border border-input bg-background px-4 py-2.5"><option value="all">Tous les statuts</option><option value="nouvelle">Nouvelles</option><option value="en_cours">En cours</option><option value="traitee">Traitees</option></select></div>
      </div>
    </div>
